...
const docQuery = path.join(__dirname, 'queryid.txt');
...
async function sendTapRequests() {
    for (const queryId of queryIds) {
        ...
        if (!userId) {
            console.error('No user id founded:', queryId);
            continue;
        }
        ...
        try {
            ...
            if (!data || !data.state || !data.token) {
                console.error('Invalid response data:', response1.data);
                continue;
            }
            console.log('===================================');
            ...
            while (checkEnergy(currentEnergy)) {
                ...
                try {
                    ...
                    if (!responseData || !responseData.state) {
                        console.log(`Tapping... ${JSON.stringify(responseData)}`);
                    }
    ...
    console.log('Take a 1-minute break before continuing...');
